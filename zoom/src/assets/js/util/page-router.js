...
/**
 * 路由
 * 升级到 1.1.0 20170415 支持page类型
 * 升级到 1.1.1 20170621 
 * 升级到 1.1.2 20170703 修改跳转页面bug
 * 升级到 1.1.3 20170906 修改切换页面 去除旧页面绑定事件
 */
...
define([ 'zepto', 'sm'], function($, sm) {
	...
	+function($) {
	    ...
	    var EVENTS = {
	    	pageSwitchStart: 'pageSwitchStart', // 
	    ...
	    var Util = {
	        ...
	        /**
	         * 判断浏览器是否支持 sessionSt，支持返回 true，否则返回 false
	         * @returns {Boolean}
	         */
	        supportStorage: function() {
	        	return $.fn.support_storage();
	        	/*
	            var mod = 'sm.router.storage.ability';
	            try {
	                sessionSt.setItem(mod, mod);
	                sessionSt.removeItem(mod);
	                return true;
	            } catch(e) {
	                return false;
	            }*/
	    ...
	    var firstInitUrl = location.href;
	    ...
	    var Router = function() {
	        ...
	        //	setTimeout(function() {
	        //		 window.addEventListener('popstate', content._onPopState.bind(content));
	        //	}, 0);
	        //});
	    };
	    
	
	    Router.prototype.isFirstInit = function() {
	       return isFirstInit;
	    };
	    
	    Router.prototype.resetFirstInit = function(flag) {
	        return isFirstInit = flag;
	    };
	    
	    Router.prototype.getFirstInitUrl = function(flag) {
	        return firstInitUrl;
	    };
	    ...
	    Router.prototype._init = function() {
	        ...
	        //    else{
	        //	if(theHistory.state.id===1){
	        //		$.fn.session_set("_back_ok", true);
			//	}
	        //}
	    ...
	    //封装url 待优化
		/*Router.prototype.wapper_url = function(url) { 
			 var _url = url;
		     var _type = $.fn.session_get('systemType');
		     var paramIndex = url.indexOf('?');
	    	 var paramsUrl = paramIndex === -1 ? '' : url.slice(paramIndex+1);
		     if (paramIndex == -1) {//不存在
		    	 ...
		    	 if(null!=paramsUrl && ''!=paramsUrl){
		    		 if(null==Util.getUrlParam(_url, "type")) _url = _url + "&type="+_type;
		    	 }else{
		    		 _url = _url + "type="+_type;
		    	 }
		     }
	     ...
	     */
	    Router.prototype.load = function(wapper_url, ignoreCache, isPushState, isReplaceState) {
	        ...
	        var content = this;
			//var wapper_url = this.wapper_url(url);
			
			this.dispatch(EVENTS.pageSwitchStart);
			
			if($.smConfig.checkBefore){
					//同步then去验证url 是否合法 token是否可以使用
					$.filterConfig.checkLogin(wapper_url).then(function (defer, data) {
						 
						 if(data && data.result==0){
						    if (content._isTheSameDocument(location.href, wapper_url)) {
					        	content._switchToSection(Util.getUrlFragment(wapper_url));
					        	$.hideMainPageLoader();
					        } else {
					        	content._saveDocumentIntoCache($(document), location.href);
					        	content._switchToDocument(wapper_url, ignoreCache, isPushState, null, isReplaceState);
					        }
						 }else if(data && data.result==2){
							 if(data.type==1){
								 if ($.smConfig.ignoreCache.allForward) {
									 $.router.load(data.href, true);
				                 }else{
				                	 $.router.load(data.href, false);
				                 }
							 }else{
	     ...
	     * @private
	     */
	    Router.prototype._switchToDocument = function(url, ignoreCache, isPushState, direction, isReplaceState) {
	        ...
	        if (cacheDocument) {
	        	this._doSwitchDocument(url, isPushState, direction, isReplaceState);
	        } else {
	            this._loadDocument(url, {
	                success: function($doc) {
	                    try {
	                        context._parseDocument(url, $doc);
	                        context._doSwitchDocument(url, isPushState, direction, isReplaceState);
	                    } catch (e) {
	                        location.href = url;
	                    }
	                },
	     ...
	     * @private
	     */
	    Router.prototype._doSwitchDocument = function(url, isPushState, direction, isReplaceState) {
	        ...
	        //console.log("$newDoc:::::"+$('<div></div>').append(this.cache[urlObj.base].$content).html());
	        ...
	        //移除旧页面的input select 等一些绑定的事件
	        $currentDoc.find('input, select, textarea').off();
	        ...
	        if(isReplaceState){
	        	this._pushOldState(url, $visibleSection.attr('id'));
	        }
	    ...
	    Router.prototype._saveDocumentIntoCache = function(doc, url) {
	        ...
	        //this.cache[urlAsKey].$content.html();
	        console.log("urlAsKey save:"+urlAsKey);
	        var $doc = $(doc);
	    ...
	    Router.prototype._getLastState = function() {
	        //var currentState = sessionSt.getItem(this.sessionNames.currentState);
	    	var currentState = $.fn.session_get(this.sessionNames.currentState);
	        try {
	    ...
	    /**
	     * 把一个状态设为当前状态，保存仅 sessionSt 中
	     *
	     * @param {State} state
	     * @private
	     */
	    Router.prototype._saveAsCurrentState = function(state) {
	        //sessionSt.setItem(this.sessionNames.currentState, JSON.stringify(state));
	        $.fn.session_set(this.sessionNames.currentState, JSON.stringify(state));
	    };
	
	    /**
	     * 获取下一个 state 的 id
	     *
	     * 读取 sessionSt 里的最后的状态的 id，然后 + 1；如果原没设置，那么返回 1
	     *
	     * @returns {number}
	     * @private
	     */
	    Router.prototype._getNextStateId = function() {
	        //var maxStateId = sessionSt.getItem(this.sessionNames.maxStateId);
	        var maxStateId = $.fn.session_get(this.sessionNames.maxStateId);
	        return maxStateId ? parseInt(maxStateId, 10) + 1 : 1;
	    };
	
	    /**
	     * 把 sessionSt 里的最后状态的 id 自加 1
	     *
	     * @private
	     */
	    Router.prototype._incMaxStateId = function() {
	        //sessionSt.setItem(this.sessionNames.maxStateId, this._getNextStateId());
	        $.fn.session_set(this.sessionNames.maxStateId, this._getNextStateId());
	    };
	    ...
	    Router.prototype._back = function(state, fromState) {
	        ...
	        if (this._isTheSameDocument(state.url.full, fromState.url.full)) {
	            ...
	            //console.log("_back:"+$.smConfig.ignoreCache.allBack);
	    ...
	    Router.prototype._onPopState = function(event) {
	        ...
	        if($.smConfig.checkBefore){
	        	...
	        	//同步then去验证url 是否合法 token是否可以使用
	    		$.filterConfig.checkLogin(state.url.full).then(function (defer, data) {
	    			 
	    			if(data && data.result==0){
	    				   
	    				if (state.id < lastState.id) {
	    					content._back(state, lastState);
	    			    } else {
	    			        content._forward(state, lastState);
	    			    } 
	    ...
	    Router.prototype._pushOldState = function(url, sectionId) {

	        var state = this._getLastState();
	        state.pageId = sectionId;
	        state.url = Util.toUrlObject(url); 
