...
;$.smVersion = "0.6.2";+function ($) {
    ...
    var defaults = {
        ...
        showPageLoadingCustomer: true, //使用自定义load
        
        router: true, //默认使用router
        animation: false, //切换页面是否动画
        ...
        ignoreCache: {
            //forward: true,
            //back: true,
            allForward: true,//true为强制加载html false使用缓存
            allBack: true
        },
...
+function ($) {
    ...
    $.confirmBiz = function (title, modalButtonOk, modalButtonCancel, callbackOk, callbackCancel) {
        return $.modal({
            text: '',
            title: typeof title === 'undefined' ? defaults.modalTitle : title,
            buttons: [
                {text: typeof modalButtonCancel === 'undefined' ? defaults.modalButtonCancel : modalButtonCancel, onClick: callbackCancel},
                {text: typeof modalButtonOk === 'undefined' ? defaults.modalButtonOk : modalButtonOk, bold: true, onClick: callbackOk}
            ]
        });
    };
    ...
    $.hideIndicator = function () {
        ...
        $('.preloader-indicator-overlay, .preloader-indicator-modal').fadeOut(300, function(){  $(this).remove(); });
    };
    //原声
    
    //摆平load 加载html使用
    $.showSwitchLoader = function(){
		if ($('.switch-loader-modal')[0]) return;
		$('body').append('<div class="switch-loader-overlay"></div><div class="switch-loader-modal"><div class="loader"></div></div>');
	};
	
	$.hideSwitchLoader = function(){
		//$('.switch-loader-overlay, .switch-loader-modal').remove();
		$('.switch-loader-overlay, .switch-loader-modal').fadeOut(300, function(){  $(this).remove(); });
	};
	...
	$.hideMainPageLoader = function(){
		...
		$(".preloader-main-indicator-overlay").fadeOut(300, function(){  $(this).remove(); });
		$(".preloader-main-indicator-modal").fadeOut(300, function(){  $(this).remove(); });
		$('.biz-indicator-overlay, .biz-indicator-modal').fadeOut(300, function(){  $(this).remove(); });
		
		if($.smConfig.showPageLoadingCustomer){
       	    $.hideSwitchLoader();
        }else{
       	    $.hideIndicator();
        }
    ...
    $.showBizIndicator = function() {
    	if ($('.biz-indicator-modal')[0]) return;
		$('.page').append('<div class="biz-indicator-overlay"></div><div class="biz-indicator-modal"><div class="loader"></div></div>');
    }
    ...
    //处理信息使用
    $.showMessageIndicator = function(title) {
    	if ($('.message-indicator-overlay')[0]) return;
    	var msgTip = $('<div style="min-width:6.82rem;padding:0.6rem 0.95rem !important;opacity:0.55;" class="message-indicator-modal preloader-indicator-modal"><div class="preloader-div"><span style="display:inline-block;width:2rem;height:2rem;" class="preloader preloader-white"></span></div><div style="text-align:center;" class="message-font">'+(title?title+'...':'正在处理中...')+'</div></div>');
    	var msgTipOverlay = $('<div class="message-indicator-overlay"></div>');
    	msgTipOverlay.append(msgTip);
    	$('.page').append(msgTipOverlay);
    	msgTip.css({
             marginLeft: (- Math.round(msgTip.outerWidth() / 2 / 1.185) - 11) + 'px' //1.185 是初始化时候的放大效果
        });
    }
    
    $.hideMessageIndicator = function() {
    	//$('.message-indicator-overlay, .message-indicator-modal').remove();
    	$('.message-indicator-overlay, .message-indicator-modal').fadeOut(100, function(){  $(this).remove(); });
    }
    ...
    //显示一个带有遮罩层的消息，2秒后消失
    $.alertMessage = function(msg, callBack, duration, extraclass){
    	var $toast = $('<div class="modal toast toast-message modal-alert ' + (extraclass || '') + '">' + msg + '</div>').appendTo($('.page'));
    ...
    /*显示一个带有遮罩层的消息，2秒后消失 是否可主动关闭*/
    $.alertClosableMessage = function(msg, callBack, duration){
    	var $toast = $('<div class="modal toast toast-message modal-alert modal-alert-close">' +'<div class="alert-msg-container">'+ msg + '</div><span class="alert-msg-toast"><a class="alert-msg-toast-ico alert-msg-toast-a">X</a></span></div>').appendTo($('.page'));
    	$('.page').append('<div class="modal-alert-overlay modal-overlay-visible modal-alert"></div>');
    	
    	$toast.find('.alert-msg-toast-a').click(function() {
			$('.toast-message').remove();
        	$('.modal-alert').remove();
        	if (typeof callBack == 'function'){
        		callBack();
    ...
    function handleClicks(e) {
    	...
    	console.log('handleClicks .... start');
        ...
        console.log('clicked hasClass:' + clicked.hasClass('modal-overlay'));
        ...
        if (clicked.hasClass('modal-overlay')) {
            ...
            var _modal = $('.modal.modal-in');
        	if (_modal && _modal.length > 0 && _modal.hasClass('modal-closeable')){
            	$.closeModal('.modal.modal-in');
            }else{
            	if (_modal && _modal.length > 0 && defaults.modalCloseByOutside){
                	$.closeModal('.modal.modal-in');
                }
            }
            
        	var _amodal = $('.actions-modal.modal-in');
         	if (_amodal && _amodal.length > 0 && _amodal.hasClass('modal-closeable')){
         		 $.closeModal('.actions-modal.modal-in');
             }else{
             	if (_amodal && _amodal.length > 0 && defaults.actionsCloseByOutside){
             		 $.closeModal('.actions-modal.modal-in');
                 }
             }
    ...
    var Calendar = function (params) {
        ...
        p.updateValue = function () {
            ...
            if (p.input && p.input.length > 0) {
                ...
                //caopeng
                if(p.params.hidenVal){
                	$(p.input).attr('date-value', inputValue);
                }else{
                ...
                if (!p.value) {
                    if (p.params.value) {
                        ...
                        //caopeng
                    	 ...
                    	 var inputVl = null;
                    	 if(p.params.hidenVal){
                    		 inputVl = $(p.input).attr('date-value');
                    	 }else{
                    		 inputVl = $(p.input).val();
                    	 }
                    	 if(inputVl){
...
+ function($) {
    ...
    var Picker = function (params) {
        ...
        var defaults = {
            ...
            toolbarTemplate: '<header class="bar bar-nav">\
                <button class="button button-link pull-right close-picker picker-confirm">确定</button>\
        ...
        p.setValue = function (arrValues, transition) {
        	...
        	if(!$.isArray(arrValues)){
        		arrValuesArray.push(arrValues);
        	}else{
        		arrValuesArray = arrValues;
        	}
        	//修复bug
            var valueIndex = 0;
            for (var i = 0; i < p.cols.length; i++) {
                if (p.cols[i] && !p.cols[i].divider) {
                    p.cols[i].setValue(arrValuesArray[valueIndex], transition);
                    valueIndex++;
                }
            }
        };
      
        // 2017/10/20 处理滚动picker赋值的问题
        p.confirmValue = function() {
            ...
            if (p.params.onConfirm) {
                p.params.onConfirm(p, p.value, p.displayValue);
            }
            if (p.input && p.input.length > 0) {
        ...
        p.open = function () {
            ...
            if (!p.opened) {
                ...
                // 绑定picker确定按钮事件
                if (!p.params.smooth) {
                	p.container.find('.picker-confirm').on('click', function () {
                		p.confirmValue();
            		});
                }
    ...
    $.fn.picker = function(params) {
        ...
        return this.each(function() {
            ...
            //待修改
            if(picker && picker.params.auto) {
            	picker.value = params.value;
                //$this.data("picker", picker);
            }
  ...
  $.lastPosition =function(options) {
    ...
    function getMemory(id,arg){
      needMemoryClass.forEach(function(item, index) {
          ...
          var positionName = id ;
          // 遍历对应节点设置存储的高度
          var memoryHeight = $.fn.session_get(positionName);
    ...
    function updateMemory(id,arg) {
        ...
        needMemoryClass.forEach(function(item, index) {
            ...
            $.fn.session_set(positionName, arg.find(item).scrollTop());
            //sessionSt.setItem(  positionName, arg.find(item).scrollTop()  );
        });
    }
  };
}(Zepto);
...
+function($) {
    ...
    if ($.smConfig.showPageLoadingIndicator) {
    	...
    	$(window).on('pageSwitchStart', function() {
            if($.smConfig.showPageLoadingCustomer){
            	 $.showSwitchLoader();
            }else{
            	 $.showIndicator();
 ...
 *
 * 通过 history.state/history.pushState 以及用 sessionSt 来记录当前 state 以及最大的 state id 来辅助前进后退的切换效果，所以在不支持 sessionSt 的情况下，将不开启路由功能。
 *
