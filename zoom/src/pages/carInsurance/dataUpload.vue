<template>
  <div class="dataUpload" :style="{ 'margin-top': $iosSystem + 45 + 'px' }">
    <div>
      ...
      <div>
        <van-tabs class="dataUpload_tabs" v-model="active">
          <van-tab title="车辆照片">
            <!-- 安卓上传图片 -->
            <van-uploader
              :after-read="(file) => afterRead(file, ture)"
              v-model="carList"
              v-if="!isIos"
            >
    ...
    <footer class="footer_btn_bar">
      <van-button
        ...
        @click="
          onSubmit(
            carList.length > 0 ? 0 : -1,
            carList.length > 0 ? -1 : papersList.length > 0 ? 0 : -1
          )
        "
        >下一步</van-button
      >
...
export default {
  ...
  created() {
    ...
    window.carInfoScanOpenAndroidCameraCallback =
      this.carInfoScanOpenAndroidCameraCallback;
    this.businessNo =
      this.$store.state.car.subProposalNo.c ||
      this.$store.state.car.subProposalNo.b ||
      ""; //交强险投保单号
    this.orderNo = this.$store.state.car.orderNo || "";
    //处理vuex保存的车辆照片
    if (
      this.phonotape.vehicle.length > 0 &&
      this.phonotape.businessNo == this.businessNo
    ) {
    ...
    //处理vuex保存的证件照片
    if (
      this.phonotape.papers.length > 0 &&
      this.phonotape.businessNo == this.businessNo
    ) {
  ...
  methods: {
    ...
    async afterRead(src, type) {
      if (type) this.goGetPic();
      var fileFlow = "";
      if (!this.isIos) {
        //安卓
    ...
    async goUploading(fileGroup, file) {
      ...
      this.commonInstance
        .post(this.$API.API_CARUPLOAD, formdata)
        .then((res) => {
      ...
      if (this.carList.length > 0 || this.papersList.length > 0) {
        if (this.carList.length > 0) {
          if (carListIndex >= 0) {
            this.goUploading(
              "YC",
              await util.compressImg(this.carList[carListIndex].file)
            );
          }
        }
        if (this.papersList.length > 0) {
          if (papersListIndex >= 0) {
            this.goUploading(
              "DZ",
              await util.compressImg(this.papersList[papersListIndex].file)
            );
          }
        }
      } else {
