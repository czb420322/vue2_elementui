...
export default {
  ...
  mounted () {
    window.openAndroidCameraCallback1 = this.openAndroidCameraCallback1;
  },
  methods: {
    // 上传行驶证
    async afterRead(src) {
    ...
    goGetCamera() {
      ...
      app.getCamera({ 'callbackMethod': 'openAndroidCameraCallback1' })
    },
    goGetPic() {
      console.log('安卓调用相册')
      this.showDialog = false;
      app.getPhoto({ 'callbackMethod': 'openAndroidCameraCallback1' })
    },
    // 摄像头回调函数
    openAndroidCameraCallback1(result) {
