        ...
        <div class="info-box">
          ...
          <template v-if="vehicleType == '1'">
            <van-field
              ...
              @click="showSelectPicker(useNatureCode)"
              readonly
              label="使用性质"
              placeholder="请选择使用性质"
              right-icon="arrow"
          ...
          <template v-if="vehicleType == '1'">
            ...
            <van-field
              ...
              @click="showSelectPicker(licenseColor)"
              readonly
              label="号牌颜色"
              placeholder="请选择号牌颜色"
              right-icon="arrow"
          ...
          <van-field
            v-if="UserInfo.manageCode == '08000000' && isNewCar"
            :value="vehicleDTO.saleAreaCode.value.text"
            label="新车销售公司所在地"
            placeholder="请选择"
            right-icon="arrow"
            readonly
            @click="showSelectPicker(vehicleDTO.saleAreaCode)"
          />
          <van-field
            v-if="UserInfo.manageCode == '08000000' && isNewCar"
            name="switch"
  ...
  created() {
    ...
    setTimeout(() => {
      if (localStorage.getItem("agreement")) {
        this.$store.state.my.newBusiness = JSON.parse(
      ...
      this.$store.commit("car/updateCarState", [
        {
          name: ["vehicleType"],
          value: this.$route.query.vehicleType == "02" ? "1" : "0",
        },
      ]);
  ...
  methods: {
    ...
    async getDecisionUnit() {
      var newData = {
        ...
        codeCode: null,
        codeName: null,
        unitmainTenanceTypeOne: null,
        unitmainTenanceTypeOneName: null,
        unitmainTenanceTypeTwo: null,
        unitmainTenanceTypeTwoName: null,
      };
      this.$store.state.my.newBusiness = newData;
      console.log(this.$store.state.my.newBusiness);
    },
    ...
    async onsubmit() {
      ...
      if (
        this.isNewCar &&
        this.UserInfo.manageCode == "08000000" &&
        !this.vehicleDTO.saleAreaCode.value.text
      ) {
      ...
      let data = {
        ...
        carOwnerDTO: {
          ...
          carOwnerIdType:
            this.carOwnerDTO.carOwnerNature.value.value == "0" ? "0" : "12",
        ...
        if (CarPrice.code == 0 && CarPrice.content.result == "0") {
          ...
          if (this.listData.cars) {
            ...
            if(this.$route.query.agentCode){
                this.$router.replace({ path: '/carVehicleInsurance?agentCode='+ this.$route.query.agentCode })
            }else{
                this.goPage("carVehicleInsurance?agentCode="+ this.$route.query.agentCode);
            }
          }
        }
      } else {
    ...
    showSelectPicker(data, str) {
      if (data) {
        ...
        this.$refs.selectPicker.parentMsg({
          struct: this.vehicleDTO.saleAreaCode,
        });
      }
    },
    ...
    updataSelectPicker(params, str) {
      ...
      if (params.name == "my/updmYdecisionUnit") {
        ...
        this.$store.commit("car/updateCarState", [
          {
            name: ["vehicleDTO", "saleAreaCode", "value"],
            value: params.struct.value,
          },
        ]);
      }
