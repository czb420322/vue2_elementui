  ...
  <div class="conditionPopupPgae">
    <van-popup v-model="show" position="bottom">
      <div class="popupContent">
        ...
        <h4>险种名称</h4>
        <ol class="select">
          <li :class="riskMark == '0801' ? 'active' : ''" @click="riskMark = '0801'">交强险</li>
          <li :class="riskMark == '0802' ? 'active' : ''" @click="riskMark = '0802'">商业险</li>
        </ol>
        <h4>终保日期</h4>
        <ol class="select">
  ...
  methods: {
    ...
    onReset() {
      ...
      this.$store.commit('renewedInsurance/updateState', [
        { name: ['car', 'query', 'renewalType'], value: '' },
        { name: ['car', 'query', 'distributeFlag'], value: '' },
        { name: ['car', 'query', 'riskMark'], value: '' },
        { name: ['car', 'query', 'endDate'], value: '' },
        { name: ['car', 'query', 'underwriteEndDate'], value: '' },
        { name: ['car', 'query', 'edDistance'], value: '' }
      ])
    },
    onConfirm() {
      this.$store.commit('renewedInsurance/updateState', [
        { name: ['car', 'query', 'renewalType'], value: this.renewalType },
        { name: ['car', 'query', 'distributeFlag'], value: this.distributeFlag },
        { name: ['car', 'query', 'riskMark'], value: this.riskMark },
        { name: ['car', 'query', 'endDate'], value: this.endDate },
        { name: ['car', 'query', 'underwriteEndDate'], value: this.underwriteEndDate },
        { name: ['car', 'query', 'edDistance'], value: this.edDistance }
      ])
      this.show = false;
    },
