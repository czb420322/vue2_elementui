...
import CVideo from './C_video' //C++播放器的方法
...
const mixins = {
    ...
    methods: {
        ...
        playvideo(row, index) {
            ...
            this.setstructureFileUuid(row.fileUuid)
            this.setshowDxplayer(true) //打开播放器
            //给当前时间赋值
        ...
        searchimg(row, zkyNum) {
            ...
            // return
            this.$http.get('vsas/structure/getFeature', {
                params: data
            }).then(res => {
                if (res.data.code == 200) {
                    let feautrue = res.data.data
                    row.Features = feautrue
                    this.setglstfeature(row)
                    this.setzkyNum(zkyNum)
                    OnExit();
                    CloseVodPlayer();
                    clearInterval(window.g_interval)
                    this.dialogVisible = false
                    console.log(this.targetToOenStrus, '222222');
                    //点击搜的时候表示是从哪一个窗口打开的
                    //1表示是从任务管理点进去的，2表示是从当前案件管理页面点进来的
                    //因为之前清理过vuex值，所以就不知道是哪一个
                    //targetToOenStrus设置表示是从哪一个页面点进来的 1 表示是任务管理 2 表示编辑页面
                    if (this.targetToOenStrus == 1 || this.$parent.isretriev == undefined) {
                        this.$emit('strusclose', false)
                        setTimeout(() => {
                        ...
                        if (this.targetToOenStrus == 0 || this.targetToOenStrus == 1) {
                            if (this.showVideoStruct == 'true') {
                                let data = {
                                    ...
                                    list: this.strusList,
                                    istwoinves: this.istwoinves,
                                    isoptfor: this.isoptfor,
                                    structuroneList: this.structuroneList,
                                    isperson: this.isperson,
                                    iscar: this.iscar,
                                    isbic: this.isbic
                                }
                                this.setstrusResult(data)
                            } else if (this.zkyStructureSwitch == 'true') {
                                let data = {
                                    personNum: this.personNum,
                                    vehicleNum: this.vehicleNum,
                                    ...
                                    istwoinves: this.istwoinves,
                                    isoptfor: this.isoptfor,
                                    structuroneList: this.zkystructuroneList,
                                    isperson: this.isperson,
                                    iscar: this.iscar,
                                    isbic: this.isbic
                                }
                    ...
                    } else if (this.targetToOenStrus == 2 || this.$parent.isretriev != undefined) {
                        setTimeout(() => {
                        ...
                        this.settargetToSearchImg(data)
                        //如果this.targetToOenStrus=0 表示是从排查之后的结果中点进去的，否则就是从第一次进入页面的排查结果点进去的
                        if (this.targetToOenStrus == 0 || this.targetToOenStrus == 2) {
                            if (this.showVideoStruct == 'true') {
                                let data = {
                                    personNum: this.personNum,
                                    vehicleNum: this.vehicleNum,
                ...
                if (res.data.code == 200) {
                    ...
                    if (this.ptlist.length) {
                        ...
                        //表示一次排查
                            ...
                            this.deviceId = JSON.parse(sessionStorage.getItem('deviceInfo')).deviceUuid
                            let type = JSON.parse(sessionStorage.getItem('deviceInfo')).type
                            this.getvideo(JSON.parse(sessionStorage.getItem('deviceInfo')).deviceUuid);
