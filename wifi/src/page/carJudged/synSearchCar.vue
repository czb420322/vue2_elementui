<template>
  <div class="wrapper">
    ...
    <div class="contentBody">
      <div class="contentLeft">
        ...
        <div class="clItem cli1">
          <div class="searchTitle">综合搜车</div>
          <ul class="searchCondition">
            ...
            <li v-show="isCarHead!==1"><span>车辆类型：</span>
              <el-select type="text" v-model.trim="vehicleKind" @change="changeVehicleKind">
            ...
            <li>
              <span>车辆品牌：</span>
              <input type="text" value="" @click="getCarBrand" readonly v-model.trim="vehicleBrand" placeholder="请输入车辆品牌"><i class="el-icon-circle-close" @click="clearBand();refresh()"></i>
            </li>
            <li v-show="isCarHead!==1">
              <span>车辆型号：</span>
              <input id="vModel" :title="vehicleModel" type="text" value="" readonly @click="getCarModule" v-model.trim="vehicleModel" placeholder="请输入车辆型号"><i class="el-icon-circle-close" @click="clear('vehicleModel')"></i>
            </li>
            ...
            <li class="carPlate"><span>车辆车牌：</span>
              <el-radio v-model="hasPlate" :label="1"> 识别车牌</el-radio>
              <el-radio v-model="hasPlate" :label="0"> 未识别车牌</el-radio>
            </li>
            <li  class="plateItem" v-show="hasPlate">
              <span>车牌号码：</span>
            ...
            <li class="carhide">
              <el-collapse v-model="activeNames">
                <el-collapse-item :title="collapseTitle" name="1">
                  <car-info :isReset="carInfoReset" @sendSynToParent="getSynDataFromChild"></car-info>
                </el-collapse-item>
              </el-collapse>
            </li>
          </ul>
      ...
      <div class="content">
        ...
        <div class="sDetail" :style="{left: checked?'0.12rem':'100%'}">
          ...
          <search-detail v-show="checked" searchType="syn" searchTypePN="synPN" searchTypeT="synT" port="/tvas/synthesizeSearch/query" :searchParams="oldParams" :searchResultDefault="searchResultDefault" :searchResultPlateNo="searchResultPlateNo" :searchResultTollgate="searchResultTollgate"></search-detail>
        </div>
    ...
    <div class="calDialog">
      <casDialog :vehicleKind="vehicleKind" :showFirst="showFirst" :showSecond="showSecond" :carBrand="vehicleBrand" :carModel="vehicleModel"  @passToParent="getChildData" :icoclick="icoclick" @changeicoclick='changeicoclick'></casDialog>
    </div>
...
<script>
  ...
  import carInfo from './carInfoComponets'
  import footerThird from '../../components/footer_third'
  export default {
    components: {
      searchDetail, selectColor, casDialog, carInfo, footerThird
    },

    data() {
      return {
        carplate:sessionStorage.getItem("provinceShort"),
        searchBtn:this.$storage.getSession("343")!=null?true:false,
        //searchPNBtn:this.$storage.getSession("344")!=null?true:false,
        //searchTBtn:this.$storage.getSession("346")!=null?true:false,
        map:null,//地图对象
        drawingManager : null,
        overlays:[],
        treeList:null,
    ...
    computed: {
      ...
      collapseTitle (){
        return this.activeNames.length === 0 ? '展开更多':"隐藏信息";
      },
      searchParams (){
        let p = {
          ...
          vehicleKind: this.isCarHead !== 1?(this.vehicleKindName=='不限'?'':this.vehicleKindName):null,
          vehicleKindCode: this.vehicleKind,
