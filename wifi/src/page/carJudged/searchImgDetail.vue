  ...
  <div class="searchDetail">
        ...
        <div class="carsList">
          <div class="carsDetail" v-for="(item,index) in actualData" :key="index">
            ...
            <div class="carsMessage">
              <div class="carsMessageTxt" v-show="isCarHead != 4 && isCarHead !=8 && isCarHead != 2 && isCarHead !=14"><span>车&nbsp;&nbsp;牌&nbsp;: &nbsp;</span><span class="plateno" :title="item.plateNo == '@'?'未识别':item.plateNo">{{item.plateNo == "@"?'未识别':item.plateNo}}</span><span style="margin-left: 0.1rem">相似度: &nbsp;</span><span class="plateno" >{{item.score?(Number(item.score)*100).toFixed(2)+'%':item.score}}</span></div>
              ...
              <div class="carsMessageTxt" v-show="isCarHead != 1 && isCarHead !=0"><span>车&nbsp;&nbsp;型&nbsp;: &nbsp;</span><span :title="item.vehicleModel">{{item.type_index | typeIndex}}</span></div>
              ...
              <div class="carsMessageTxt oprList" v-show="isCarHead != 0 && isCarHead !=1">
                <span>快捷操作&nbsp;: &nbsp;</span>
                  <span title="图片下载" class="opr opr9" @click="downImg(index,item)"></span>
                  <span v-show="imgSearchBtn" class="opr opr1" title="以图搜车" @click="goToSearchImg(item.imageUrl)"></span>
                </div>
    ...
    <el-dialog
        ...
        <el-carousel ref="carouselEl" id="carouselEl" height="7rem" arrow="never" :autoplay="false" indicator-position="none" @change="changeActive">
          <el-carousel-item v-for="(item,index) in srD.data.rows" :key="index">
            <div class="carouselImg" :id="'cImg_'+index">
              <img :id="'cImgItem_'+index" :src="item.imageUrl" />
              <div class="facelist" :id="'faceList_'+index">
                <span class="faceItem" v-for="(faceItem,index) in faceStyle[index]" :key="index"><img :style="{'top':-faceItem.top+'px', 'left': -faceItem.left+'px'}" :width="faceItem.width" :height="faceItem.height" :src="item.imageUrl" /></span>
              </div>
              <span :id="'cImgRedBorder_'+index" class="carsRedBorder"></span>
            </div>
          </el-carousel-item>
        ...
        <div class="information" v-if="carousel">
          ...
          <div class="infomain">
            <div class="item" v-show="isCarHead != 4 && isCarHead !=8 && isCarHead != 2 && isCarHead !=14">车型：{{srD.data.rows[Indexs].vehicleModel}}</div>
            <div class="item" v-show="isCarHead != 0 && isCarHead !=1">车型：{{srD.data.rows[Indexs].type_index| typeIndex}}</div>
    ...
    <form action="tvas/downloadFile/downloadImage" method="post" v-for="(item,index) in  srD.data.rows" :key="index" :id="'a'+index" v-show="false" >
      ...
      <form action="tvas/downloadFile/batchDownloadImage" method="post" id="downAll"  v-show="false" >
       ...
       <!-- <form action="tvas/downloadFile/batchDownloadImage" method="post" id="downAllmor"  v-show="false" >
        <input type="text" :value="this.searchParams.vehicleKind" name="vehicleKind">
        <input type="text" :value="this.searchParams.vehicleBrand" name="vehicleBrand">
        <input type="text" :value="this.searchParams.vehicleFeature" name="vehicleFeature">
        <input type="text" :value="this.searchParams.isCarHead" name="isCarHead">
...
<script>
  ...
  export default {
    ...
    computed:{
      ...
      //   dParams.pageNo = this.srD.data.pageNo;
      //   dParams.pageSize = this.srD.data.pageSize;
      //   dParams.orderBy = this.orderBy;
      //   dParams.orderType = this.orderType;
      //   return dParams;
      // },
      ...
      faceStyle (){
        let arr = [];
        let dList = this.srD.data.rows;
        if(dList.length && (this.isCarHead == 1 || this.isCarHead == 0)){
          for(let i=0;i<dList.length;i++){
            let arrItem = [];
    ...
    watch: {
      searchResultDefault: {
        handler(newValue, oldValue) {
          ...
          this.pageNo = 1;
          this.pageSize = 10;
          this.otherPort = this.port  //接口
          this.total = this.srD.data.rows.length;
          this.actualData = this.srD.data.rows.slice((this.pageNo-1)*this.pageSize, this.pageNo*this.pageSize);
        },
        deep: true
      }
    },
    ...
    methods: {
      ...
      openCarousel(){
        this.$nextTick(function () {
          if(this.srD.data.rows.length && this.firstOpen){
            this.redBorderStyle(0, this.srD.data.rows[0]);
    ...
    mounted() {
      ...
      this.pageSize = 10;
      this.total = this.srD.data.rows.length;
      this.actualData = this.searchResultDefault.data.rows.slice((this.pageNo-1)*this.pageSize, this.pageNo*this.pageSize);
