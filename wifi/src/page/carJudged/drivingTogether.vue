<template>
  <div class="wrapper">
    ...
    <div class="contentBody">
      ...
      <div class="contentLeft">
        ...
        <div class="clItem cli1">
          <div class="searchTitle">同行车分析</div>
          <ul class="searchCondition">
            ...
            <li class="backItem"><span>跟车车牌类型：</span>
              <el-select type="text" v-model="plateClass">
                <el-option :value="null" label="不限"></el-option>
                <el-option v-for="(item ,index) in plateClassList" :value="item.field" :label="item.value" :key="index"></el-option>
              </el-select></li>
            <li class="ftcItem">
              <div class="left_ftci" ><span class="bt">*</span>跟车时段：</div>
            ...
            <li class="followItem">
              <span><span class="bt">*</span>跟车时间：</span>
              <span class="lt">小于</span><input class="followTimeIn" type="text" ref="followTime" v-model.trim="followTime"  placeholder="请输入跟车时间" @blur="checkFollowTime"><i class="el-icon-circle-close" @click="clear('followTime')"></i><span class="unit">分钟</span>
            </li>
            <li class="followItem">
              <span><span class="bt">*</span>跟车次数：</span>
              <span class="lt">大于</span><input class="followTimeIn" type="text" ref="followNumber" v-model.trim="followNumber" placeholder="请输入跟车次数" @blur="checkFollowNumber"><i class="el-icon-circle-close" @click="clear('followNumber')"></i><span class="unit">次</span>
            </li>
            <li>
              <span>车辆品牌：</span>
      ...
      <div class="content">
        ...
        <div class="sDetail" :style="{left: checked?'0.12rem':'100%'}">
          ...
          <first-to-city-detail v-show="checked" searchType="follow" port="/tvas/synthesizeAnalyze/follow/query" :searchParams="oldParams" :searchResult="searchResult" ></first-to-city-detail>
        </div>
    ...
    <div class="err" v-if="Err.followTime.flag" :style="{top: Err.followTime.top}">{{Err.followTime.txt}}</div>
    <div class="err" v-if="Err.followNumber.flag" :style="{top: Err.followNumber.top}">{{Err.followNumber.txt}}</div>
...
<script>
  ...
  export default {
    components: {
      casDialog, firstToCityDetail, footerThird
    },

    data() {
      return {
        carplate:sessionStorage.getItem("provinceShort"),
        searchBtn:this.$storage.getSession("314")!=null?true:false,
        map:null,//地图对象
        drawingManager : null,
        overlays:[],
        treeList:null,
        ...
        loading: null,
        followMaxDays: sessionStorage.getItem("followMaxDays"), //可配置时间
    ...
    plateClass(newValue,oldValue){
        if(newValue!=null){
          this.getplateClassName()
        }else{
          this.plateClassName='不限'
        }
    ...
    computed: {
      ...
      searchParams(){
        return {
          ...
          plateClass: this.plateClassName=='不限'?'':this.plateClassName,
          plateClassCode: this.plateClass,
          plateNo: this.platNumArr.join(','),
          followTime: this.followTime,
          followNumber: this.followNumber,
    ...
    changeicoclick(data){
      ...
      getplateClassName(){
      this.plateClassList.map((item,idx)=>{
            if(item.field==this.plateClass){
              this.plateClassName=item.value
            }
      ...
      reset() {
        ...
        let minDates = new Date().getTime() - (Number(this.followMaxDays)-1)*24*60*60*1000;
    ...
    mounted: function () {
      this.getPlateClassType((res)=>{
        this.plateClassList = res.data.data;
      });
      this.getWeekDay();
      this.getTreeData();
      this.getCheckedItem(["beginDateTime", "endDateTime", "followTime","followNumber","plateNo"]);
      ...
      this.beginDateTime = this.getDateString(new Date().getTime() - (Number(this.followMaxDays)-1)*24*60*60*1000).split(' ')[0] + " " + "00:00:00";
      this.timeBegin = laydate.render({
        ...
        btns: ['now','confirm'],
          ...
          let time = Number(_this.followMaxDays)-1;
