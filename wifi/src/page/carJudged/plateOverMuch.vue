    ...
    <div class="contentBody">
      ...
      <div class="contentLeft">
        ...
        <div class="clItem cli1">
          <div class="searchTitle">一牌多车分析</div>
          <ul class="searchCondition">
            ...
            <li><span>车牌类型：</span>
              <el-select type="text" v-model="plateClass" placeholder="请选择车辆类型">
      ...
      <div class="content">
        ...
        <div class="sDetail" :style="{left: checked?'0.12rem':'100%'}">
          ...
          <div class="searchDetailFH">
            <div class="resultTitle">| 一牌多车搜索</div>
            <div class="resultList">
              ...
              >
                  <template slot-scope="scope">
                  <span @click="getDetail(scope.row.plateNo,scope.row.plateClassCode,scope.row.plateColor)"><span class="searchBtn">查看详情</span> </span>
                </template>
              </el-table-column>
            ...
            <!--  <table class="resultTable">
                ...
                <tr v-for="(item, index) in actualData">
                  ...
                  <td>{{item.plateNo == "@"?null:item.plateNo}}</td>
                  <td>{{item.plateClass}}</td>
                  <td>{{item.plateColor}}</td>
                  <td>{{item.vehicleCount}}</td>
                  <td @click="getDetail(item.plateNo, item.plateClassCode,item.plateColor)"><span class="desc">查看详情</span></td>
                </tr>
                <tr v-if="!actualData.length">
                  <td colspan="6">暂无数据</td>
                </tr>
              </table>-->
              <div class="pageNation oftenPassPage">
                <el-pagination
                  :background=true
          ...
          <!--一牌多车列表下某项详细-->
          <car-detail :isShow="showCarDetail" type="moreCarD" :carParams="currentParams" :carResult="searchResultDetail" @closeCarDetail="closeCarDetail"></car-detail>
        </div>
      </div>
...
<script>
  ...
  export default {
    components: {
      carDetail, footerThird
    },

    data() {
      return {
        carplate:sessionStorage.getItem("provinceShort"),
        searchBtn:this.$storage.getSession("310")!=null?true:false,
        //searchDetailBtn:this.$storage.getSession("353")!=null?true:false,
        map:null,//地图对象
        drawingManager : null,
        overlays:[],
        treeList:null,
        ...
        loading: null,
        morecarMaxDays: sessionStorage.getItem("morecarMaxDays"), //可配置时间
    ...
    methods: {
      ...
      //     text: "加载中"
      //   });
      //   this.getVD("moreCar", this.searchParams,(res)=>{
      ...
      getCarData (){
        ...
        this.getVD("moreCar", params3, (res)=>{
          this.transValue(res.data, this.searchResult);
      ...
      handleSizeChange(val) {
        ...
        this.oldParams.pageNo = this.actualPageNo;
        this.oldParams.pageSize = this.actualPageSize;

        this.getVD("moreCar", this.oldParams,(res)=>{
          this.transValue(res.data, this.searchResult);
          if(this.searchResult.data.rows.length === 0 ) {
             this.$notify({
                        type: 'info',
      ...
      getPlateColor(){
        this.$http.get('sms/static/getTypeByCode?code=PlateColorType').then(res => {
          if (res.data.code == 200) {
            ...
            console.log(this.PlateColorType,999);
          }
        })
      },

      getDetail (pId,plateClass,plateColor) {
        for (let i = 0; i < this.PlateColorType.length; i++) {
          if(plateColor == this.PlateColorType[i].value){
            this.plateColorCode = this.PlateColorType[i].field
          }
        }
        ...
        this.currentParams = {
          ...
          plateClass: this.oldParams.plateClass,
          plateClassCode: plateClass,
          plateColor: this.plateColor,
          plateColorCode: this.plateColorCode,
        ...
        this.getVD("moreCarD", this.currentParams, (res)=>{
    ...
    mounted: function () {
      ...
      this.timeBegin = laydate.render({
        ...
        btns: ['now','confirm'],
          ...
          let time = Number(_this.morecarMaxDays)-1;  //可配置时间
