    ...
    <!-- <div class="topInfo">
     ...
     <div class="topInfo">
          ...
          <span class="sysname childname">当前位置：</span>
           <p class="sysname" @click="gohome">系统管理 > </p>
          <p class="sysname">&nbsp;特征管理</p>
      </div>
    <div class="dicManageWrappr">
      <div class="content">
        <div class="contentTabs">
          <el-select  @change="tabsChange" v-model="tabsValue">
        ...
        <div class="contentDetialWrapper">
          <div class="picModel">
            <div v-if="switchPcitureFlag" style="width: 100%;height: 100%">
              <div class="picModelTitle">
                ...
                <div class="titleTxt">人员特征模型</div>
              </div>
              <div class="picModelPicture">
                <img :src=personImgSrc usemap="#amap" style="width:775px;height:515px" @load="calculatPicture(percent)" alt="" id="personImg">
          ...
          <div class="modelPart">
            <div class="modelPartTitle">
              <div class="titleLine"></div>
              <div class="titleTxt">特征部位选择</div>
            </div>
            <div class="modelPartCondition">
              <div class="conditionChoose">
                <div class="chooseItem1"><span>选择部位: &nbsp;</span><input v-model="partName" readonly type="text"></div>
                ...
                <div class="chooseItem2">
                  <span>描述说明 :&nbsp; </span>
                  <input type="text" v-model="desc" maxlength="20" @blur="updateItem" />
                </div>
              </div>
              <div class="conditionList">
               <el-table :data="descriptionList" height="3rem" border style="width: 100%" class="tables">
                  <div slot="empty">
                    <div class="noImg">
                  ...
                  <el-table-column show-overflow-tooltip label="部位" width="180">
                  ...
                  <el-table-column prop="operation" label="操作">
                    <template slot-scope="scope">
                        <div class="el-icon-edit tryBtn" @click="editCell(scope.row)"></div>
    ...
    methods: {
      ...
      getPartMessage(positionId) {
        this.$http({
          ...
          url:"sms/threeData/loadThreeByPosition",
          method: 'get',
          params: {
      ...
      addItem(){
        if(this.desc == "" || this.desc == null){
          // layer.alert('部位描述必填！',{icon:5,title:"消息提示"});
          this.$notify({
            type: 'warning',
            message: '部位描述必填！',
            position: 'bottom-right',
        ...
        if(this.descriptionList.findIndex(c=>{
            return c.description == this.desc
        }) != -1){
            this.$notify({
                type: 'warning',
                message: '请勿添加重复数据！',
        ...
        this.$http({
          url: 'sms/threeData/addThreeData',
          method: 'post',
          data: data
        }).then(res => {
          if (res.data.code === 200) {
            ...
            this.getPartMessage(this.positionId);
            this.desc = "";
            this.isEdit = false;
            ...
            // layer.alert('保存部位描述失败！',{icon:5,title:"消息提示"});
          }
        })
      },
      //获取搜索条件列表
      getSearchData() {
        ...
        let decription = this.$refs.partDescription.options[selectedIndex].text;
      ...
      editCell(row){
        ...
        this.threeDataId = row.threeDataId;
      },
      saveEditCell(){
        if(this.desc == "" || this.desc == null){
           this.$notify({
        ...
        let params = {
          threeDataId: this.threeDataId,
          positionId: this.positionId,
          description:this.desc
        };
        let data = this.$qs.stringify(params);
        this.$http({
          url: 'sms/threeData/updataThreeData',
          method: 'post',
          data: data
        }).then(res => {
          if (res.data.code === 200) {
            ...
            // layer.alert('编辑部位描述成功！',{icon:6,title:"消息提示"});
            this.getPartMessage(this.positionId);
            this.desc = "";
            this.isEdit = false;
          } else if (res.data.code === 100) {
            ...
            // layer.alert('编辑部位描述失败！',{icon:5,title:"消息提示"});
          }
        })
      },
      //删除条件
      deletCell(row) {
        let params = {
          threeDataId: row.threeDataId,
          positionId: this.positionId
        };
        let data = this.$qs.stringify(params);
        this.$http({
          url: 'sms/threeData/deleteThreeData',
          method: 'post',
          data: data
        }).then(res => {
          if (res.data.code === 200) {
            ...
            //   }
            // })
            // layer.alert('删除部位描述成功！',{icon:6,title:"消息提示"});
            this.getPartMessage(this.positionId);
          }else{
             this.$notify({
                type: 'error',
            ...
            //   }
            // })
            // layer.alert('删除部位描述失败！',{icon:5,title:"消息提示"});
          }
        })
      },
    }
...
.contentTabs >>> .el-input__inner{
  ...
  .titleLine{
    width: 2px;height:0.14rem;background-color:#fff;margin-right: 0.1rem;margin-top: 3px
  }
  .titleTxt{
