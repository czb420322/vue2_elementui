 ...
 * @LastEditTime : 2021-06-04 09:58:48
          ...
          <el-tab-pane label="对接设置" name="first">
            <div class="navBar-top">
              ...
              <div class="navBar-footer allequally">
                ...
                <div class="btnOne">
                  <el-button type="primary" class="searchBtn" size="small" @click="gbssearchBtn">提交</el-button>
        ...
        <el-tabs v-model="activeName" @tab-click="handleClick" v-if="showGbhss">
          <el-tab-pane label="基本参数配置" name="sixth">
            <div class="navBar-gbhstop">
              ...
              <div class="btnTwo">
                <el-button type="primary" class="searchBtn" size="small" @click="gbhsssearchBtn">保存</el-button>
              ...
              <div class="navBar-center gbreally">
                <ul>
                  ...
                  <li>
                    ...
                    <input type="text" v-model="segmentTimeSeconds"><i> 秒 </i>
                  </li>
                  <li>
                    <span>
                      国标拉流超时时间：
    ...
    realtimeData() {
      this.$http.post("sms/skynet/realTimeQuery").then(res => {
        if (res.data.code == 200) {
          ...
          this.gbPullerRealtimeStoreFilePath = res.data.data.gbPullerRealtimeStoreFilePath; //拉流视频临时存储路径
          this.gbPullerRealtimeIsSaveFile = res.data.data.gbPullerRealtimeIsSaveFile; //是否保存拉流视频文件; 1：保存，0：不保存。
          this.getDeviceStatusByProxy = JSON.stringify(res.data.data.getDeviceStatusByProxy); //是否启用代理接口获取设备在线状态，默认1。1：代理接口获取设备在线状态，0：为一直在线模式
