 ...
 * @LastEditTime : 2021-01-22 10:07:56
...
const mixins = {
    ...
    methods: {
        ...
        // 一次排查 查询
        //播放playtype类型，1表示下一个播放，2表示上一个播放
        getSearchZkySize(changeFlag,playtype) {
            this.changeFlag = 1
            ...
            this.$http.post('vsas/structure/oneFilterZky', datas).then(res => {
                    ...
                    if (res.data.code == 200) { // 获取数据
                        ...
                        //表示++
                        if(playtype == 1){
                            this.$emit('childByPlay',1)
                        }
                        else if(playtype == 2){
                            this.$emit('childByPlay',2)
                        }
        ...
        // 二次排查 查询
        //playtype 1.下一个，2上一个
        getTwozkySize(changeFlag,playtype) {
            this.changeFlag = 2
            let times = this.getinvesTime()
            let direction
            if (this.arearule == 4) {
