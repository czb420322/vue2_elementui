      ...
      <div class="content">
        <div class="top">
          ...
          <div class="topContent">
            <ul class="caseDetial">
              <li>
                <span style="width: 70px">串并案名称:</span> <input readonly="readonly" type="text" style="width: calc(100% - 56px);" :placeholder="lookCaseObj.comboCaseName">
              </li>
              <li style="display: flex">
                ...
                <div style="width:60%;"><span>创建时间:&nbsp;&nbsp;</span><span v-text="lookCaseObj.comboCreateTime">&nbsp;&nbsp;2018-02-08  12:00:00</span></div>
              </li>
              <li>
                <div style="width:40%;"><span style="width: 55px">创建用户:&nbsp;&nbsp;</span><span v-text="lookCaseObj.comboCreatorName">&nbsp;&nbsp;</span></div>
                <div style="width:60%;"><span >创建部门:&nbsp;&nbsp;</span><span  v-text="lookCaseObj.comboCreatorOrgName">&nbsp;&nbsp;</span></div>

              </li>
              <li>
                <span style="width: 70px;">串并案描述:</span>
                &nbsp;<textarea readonly="readonly" name="" style="width:calc(100% - 56px);height: 0.7rem;;margin-left: -2px" :placeholder="lookCaseObj.comboDescription"></textarea>
              </li>

               <li><span style="width: 70px;">审批结果:</span>
                    <span v-if="lookCaseObj.comboCaseState==0">新建</span>
                    <span v-if="lookCaseObj.comboCaseState==1">审批中</span>
                    <span v-if="lookCaseObj.comboCaseState==2">审批通过</span>
                    <span v-if="lookCaseObj.comboCaseState==3">审批不通过</span>
                    <span  v-else></span>
                </li>
        ...
        <div class="bottom">
          ...
          <div class="bottomDetail">
            <div  v-for="(item,index) in historyCaseList" class="detailItem" :key="index">
              <div class="detailItemImg" v-if="lookCaseObj.comboCaseState == 2" @click="lookCase(item)">
               ...
               <div class="detailItemImg" v-if="lookCaseObj.comboCaseState == 3" @click="lookCase(item)">
                <img style="width: 100%;height: 100%" :src="item.caseImgUrl == null ? $store.state.defaultImg : item.caseImgUrl" :onerror="$store.state.defaultImg" alt="">
                <!-- <img style="width: 100%;height: 100%" :src="item.caseImgUrl" :onerror="$store.state.defaultImg" alt=""> -->
              </div>
              <div class="detailItemTxt">
                <div class="detailItemTxtTitle" :title="item.caseName">案件名称: <span v-text="item.caseName"></span></div>
                <div class="detailItemTxtItem">
                  <div class="detailItemTxtItemLeft">
                    ...
                    <span v-show="showCase == 2 || showCase == 1">审批状态: {{item.approvalStatus | caseApprove}}</span></div>
                  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      getCaseObj() {
        this.$http({
          url: 'pca/combo/myCombo/caseComboQuery',
          method: 'get',
          params: {
      ...
      getCaseInfo(){
        this.$http({
          url:'pca/combo/myCombo/getCaseInfoByApproveId',
          method:'get',
          params:{
            approveId:this.approveId
          }
      ...
      goBack(){
        this.$router.push('/casestudy/cascadeApprove?menuCode=211&his=true')
      },
      ...
      initCasePoint() {
        ...
        for (let i = 0; i < this.newpoints.length; i++) {
          ...
          console.log(point,7777777777777);
      ...
      //跳转查看案件
      lookCase(obj) {
        if(this.lookCaseObj.comboCaseState == 3){
          this.$notify({
            type: 'error',
            message: '当前串并案审批不通过，不允许查看案件详情',
            position: 'bottom-right',
            duration: 3000
          });
          return
