<template>
  <div class="caseinfo">
    ...
    <div class="topInfo">
      ...
      <router-link
        class="sysname"
        :to="{path:'/casestudy',query:{menuCode:200}}">案件库 > </router-link>
      <router-link
        tag="div"
        :to="{path:'/casestudy/caseInfomanage',query:{menuCode:202}}"
        class="sysname"
        v-if='caseInfom'
      >&nbsp;案事件信息管理 > </router-link>
      <router-link
      ...
      <router-link  tag="div" :to="{path:'/casestudy/caseInfomanage',query:{menuCode:202}}" class="sysname sysnamesecond" v-if='caseInfom'>案事件信息管理</router-link>
      <router-link  tag="div" :to="{path:'/casestudy/myupload',query:{menuCode:203}}" class="sysname sysnamesecond" v-if='myUploadTo'>我的案事件</router-link>
      <router-link  tag="div" :to="{path:'/casestudy/mysynergy',query:{menuCode:222}}" class="sysname sysnamesecond" v-if='mysynergy'>我的协同</router-link>
      <div class="sysname childname addcaseinfo">案件信息</div> -->
    </div>
          ...
          <div class="main_top">
            <div class="main_tleft">
              ...
              <ul class="mainInfo">
                <li>
                  ...
                  >
                  <div
                    class="mustText mustText0"
                    v-show="caseNumberMust"
                  >该案件编号已存在<i></i></div>
                </li>
                <li>
                ...
                <li class="">
                  <span>协同作战：</span>
                  <el-switch
                    :disabled="synergyCombat == 0 ? false : true"
                    v-model="synergyCombat"
                    active-color="#13ce66"
            ...
            <div class="main_tright">
              ...
              <img
                ...
                @click="InitMainMap('mapContent');ajaxEditList()"
              />
            </div>
          </div>
          <div class="main_btn">
            <el-button
              class="cancleBtn"
              @click="edit ? editReast() : reast() "
            >{{ edit ? '重置到编辑前':'重置' }}</el-button>
            <el-button
              class="cancleBtn"
              @click="goToprev"
            >上一步</el-button>
  ...
  methods: {
    ...
    getCaseNumber() {
      ...
      if (this.caseNumber != "") {
        ...
        this.$http
          .get("/pca/casemng/baseinfo/check/CaseLinkMark", { params: data })
          .then(res => {
    ...
    ajaxInformation() {
      ...
      if (this.caseName == "") {
        ...
        console.log(this.caselng,1111);
        this.$http.post("pca/casemng/baseinfo/save", data).then(response => {
          if (response.data.code == 200) {
            if (!this.synergyCombat) {
              //不是协同作战\
              if (this.caseInfom) {
                this.$router.push({
                  path: "/casestudy/caseInfomanage/addcasetwo?menuCode=200",
                  query: {
                    caseUuid: response.data.data.caseUuid,
                    caseInfom: true,
                    edit: this.edit,
                    hide: false,
                    synergyCombat: this.synergyCombat
                  }
                });
              } else if (this.myUploadTo) {
                this.$router.push({
              ...
              if (this.myUploadTo) {
                this.$router.push({
                  path: "/casestudy/caseInfomanage/addsyncase?menuCode=200",
                  query: {
                    caseUuid: response.data.data.caseUuid,
                    edit: this.edit,
                    myUploadTo: true,
            ...
            // layer.alert('请求失败，服务器异常！',{icon:2,title:"消息提示"})
          }
        });
      }
    },
        ...
        this.$http
          ...
          .then(res => {
            if (res.data.code == 200) {
              ...
              this.$refs.province.selected = res.data.data.placeCode.substring(0,2) + '0000'
              this.$refs.province.selectcity = res.data.data.placeCode.substring(0,4) + '00'
              this.$refs.province.selectdist = res.data.data.placeCode
              this.$refs.province.getSelectData();
...
.maintile {
  ...
  line-height: 0.5rem;
  color: #fff;
  border: 1px solid rgba(27, 55, 88, 0.5);
}
.maintile span {
  font-size: 0.24rem;
  font-weight: 700;
