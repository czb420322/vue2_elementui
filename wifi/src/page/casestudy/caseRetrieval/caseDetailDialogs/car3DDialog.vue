      ...
      <div class="dialogContainer">
        ...
        <div class="dialogDetail">
          <div class="detailItemLeft">
            <div class="natureName">车辆特征属性</div>
            <!-- <div class="natureName naturePicture">案件截图</div> -->
            <div class="natureName">案件名称</div>
            <div class="natureName">案发时间</div>
            <div class="natureName">雨刷器</div>
            <div class="natureName">车标</div>
            <div class="natureName">风窗玻璃</div>
            <div class="natureName">前照灯</div>
            <div class="natureName">尾灯</div>
            <div class="natureName">转向灯</div>
            <!-- <div class="natureName">侧车灯</div> -->
            <div class="natureName">车牌号</div>
            <div class="natureName">前雾灯</div>
            <div class="natureName">后雾灯</div>
            <div class="natureName">后视镜</div>
            <div class="natureName">保险杠</div>
            <div class="natureName">发动机舱盖</div>
            <div class="natureName">前车轮</div>
            <div class="natureName">后车轮</div>
            <div class="natureName">门把手</div>
            <div class="natureName">车门</div>
            <div class="natureName">车窗</div>
            <div class="natureName">天窗/车顶</div>
            <div class="natureName">行李盖</div>
            <div class="natureName">油箱盖</div>
            <div class="natureName">排气孔</div>
            <div class="natureName">翼子板</div>
            <div class="natureName">车身</div>
          ...
          <div class="detailItemRight">
            <div class="ownNature">
              ...
              <div class="natureItem">{{ isEmpty(car3DObj.wiper) }}</div>
              <div class="natureItem">{{ isEmpty(car3DObj.vehicleMark) }}</div>
              <div class="natureItem">{{ isEmpty(car3DObj.windowglass) }}</div>
              <div class="natureItem">{{ isEmpty(car3DObj.headlamp) }}</div>
              <div class="natureItem">{{ isEmpty(car3DObj.taillight) }}</div>
              <div class="natureItem">{{ isEmpty(car3DObj.turningLamp) }}</div>
              <div class="natureItem">{{ isEmpty(car3DObj.carNumber) }}</div>
              <div class="natureItem">{{ isEmpty(car3DObj.frontFogLamp) }}</div>
              <div class="natureItem">{{ isEmpty(car3DObj.rearFogLamp) }}</div>
              <div class="natureItem">{{ isEmpty(car3DObj.rearviewMirror) }}</div>
              <div class="natureItem">{{ isEmpty(car3DObj.bumper) }}</div>
              <div class="natureItem">{{ isEmpty(car3DObj.engineHatchCover) }}</div>
              <div class="natureItem">{{ isEmpty(car3DObj.frontWheel) }}</div>
              <div class="natureItem">{{ isEmpty(car3DObj.rearWheel) }}</div>
              <div class="natureItem">{{ isEmpty(car3DObj.doorHandle) }}</div>
              <div class="natureItem">{{ isEmpty(car3DObj.carDoor) }}</div>
              <div class="natureItem">{{ isEmpty(car3DObj.window) }}</div>
              <div class="natureItem">{{ isEmpty(car3DObj.skylightRoof) }}</div>
              <div class="natureItem">{{ isEmpty(car3DObj.luggageCover) }}</div>
              <div class="natureItem">{{ isEmpty(car3DObj.oilTankCover) }}</div>
              <div class="natureItem">{{ isEmpty(car3DObj.exhaustHole) }}</div>
              <div class="natureItem">{{ isEmpty(car3DObj.wingedPlate) }}</div>
              <div class="natureItem">{{ isEmpty(car3DObj.body) }}</div>
              <div class="natureItem"></div>
            </div>
            <div class="othersNature" v-for="(item, index) in compareList"  :key="index">
              ...
              <div class="natureItem">{{ isEmpty(item.wiper) }}</div>
              <div class="natureItem">{{ isEmpty(item.vehicleMark) }}</div>
              <div class="natureItem">{{ isEmpty(item.windowglass) }}</div>
              <div class="natureItem">{{ isEmpty(item.headlamp) }}</div>
              <div class="natureItem">{{ isEmpty(item.taillight) }}</div>
              <div class="natureItem">{{ isEmpty(item.turningLamp) }}</div>
              <div class="natureItem">{{ isEmpty(item.carNumber) }}</div>
              <div class="natureItem">{{ isEmpty(item.frontFogLamp) }}</div>
              <div class="natureItem">{{ isEmpty(item.rearFogLamp) }}</div>
              <div class="natureItem">{{ isEmpty(item.rearviewMirror) }}</div>
              <div class="natureItem">{{ isEmpty(item.bumper) }}</div>
              <div class="natureItem">{{ isEmpty(item.engineHatchCover) }}</div>
              <div class="natureItem">{{ isEmpty(item.frontWheel) }}</div>
              <div class="natureItem">{{ isEmpty(item.rearWheel) }}</div>
              <div class="natureItem">{{ isEmpty(item.doorHandle) }}</div>
              <div class="natureItem">{{ isEmpty(item.carDoor) }}</div>
              <div class="natureItem">{{ isEmpty(item.window) }}</div>
              <div class="natureItem">{{ isEmpty(item.skylightRoof) }}</div>
              <div class="natureItem">{{ isEmpty(item.luggageCover) }}</div>
              <div class="natureItem">{{ isEmpty(item.oilTankCover) }}</div>
              <div class="natureItem">{{ isEmpty(item.exhaustHole) }}</div>
              <div class="natureItem">{{ isEmpty(item.wingedPlate) }}</div>
              <div class="natureItem">{{ isEmpty(item.body) }}</div>
        ...
        <span slot="footer" class="dialog-footer">
          ...
          <el-button class="passBtn" @click="checkcaseList()">保存串并</el-button>
      </span>
          <el-dialog
            width="30%"
            title="串并案保存"
  ...
  export default {
    ...
    methods: {
      ...
      getCar3DObj(){
        ...
        this.$http.post('pca/target/comparison/carThree',data)
        .then (res =>{
          console.log('获取原点案件车辆特征信息'+JSON.stringify(res))
      ...
      getCompareList() {
        ...
        data = this.$qs.stringify(data);
        this.$http.post('pca/target/comparison/carthreeSuspicionCase',data)
        .then(res => {
    ...
    computed: {
      dialogVisible: {
        get: function () {
          return this.$store.state.car3DDialog
        },
        set: function (newVal) {
          this.$store.state.car3DDialog = newVal
        }
      }
    },
    watch: {
