  ...
  <div class="dialogWrapper">
    ...
    >
      <div class="dialogContainer">
        ...
        <div class="dialogDetail">
          <div class="detailItemLeft">
            <div class="natureName">车辆属性</div>
            <!-- <div class="natureName naturePicture">线索图片</div> -->
            <div class="natureName">案件名称</div>
            <div class="natureName">案发时间</div>
            <div class="natureName">目标车牌种类</div>
            <div class="natureName">目标车牌颜色</div>
            <div class="natureName">目标车辆号牌</div>
            <div class="natureName">目标车辆颜色</div>
            <!-- <div class="natureName">目标可疑行为</div>
            <div class="natureName">目标前部物品类型</div>
            <div class="natureName">目标后部物品类型</div>
            <div class="natureName">目标车模颜色</div>-->
          ...
          <div class="detailItemRight">
            <div class="ownNature">
              ...
              <!-- <div class="natureItem naturePicture cursor" @click="lookCase(dataCaseInfo.caseUuid)">
                <img style="width: 100%;height: 100%" :src="carNatureObj.traceImgUrl" alt="暂无图片" v-show="carNatureObj.traceImgUrl">
                <img style="width: 100%;height: 100%" src="@/assets/error_none.png" alt="暂无图片" v-show="!carNatureObj.traceImgUrl">
              </div>-->
              ...
              <div class="natureItem">{{ isEmpty(carNatureObj.plateClass) }}</div>
              <div class="natureItem">{{ isEmpty(carNatureObj.plateColor) }}</div>
              <div class="natureItem">{{ isEmpty(carNatureObj.plateNo) }}</div>
              <div class="natureItem">{{ isEmpty(carNatureObj.vehicleColor) }}</div>
              <!-- <div class="natureItem">{{ isEmpty(carNatureObj.behavior) }}</div>
              <div class="natureItem">{{ isEmpty(carNatureObj.vehicleFrontItem) }}</div>
              <div class="natureItem">{{ isEmpty(carNatureObj.vehicleRearItem) }}</div>
              <div class="natureItem">{{ isEmpty(carNatureObj.filmColor) }}</div>-->
            ...
            <div class="othersNature" v-for="(item, index) in compareList" :key="index">
              ...
              <!-- <div class="natureItem naturePicture cursor" @click="lookCase(item.caseUuid)">
                  <img style="width: 100%;height: 100%" :src="item.motorVehicle.traceImgUrl" alt="暂无图片" v-show="item.motorVehicle.traceImgUrl">
                  <img style="width: 100%;height: 100%" src="@/assets/error_none.png" alt="暂无图片" v-show="!item.motorVehicle.traceImgUrl">
              </div>-->
              <div class="natureItem">
                <span :title="item.caseName">{{ item.caseName }}</span>
              </div>
              <div class="natureItem">{{ item.timeLowLimit }}</div>
              <div class="natureItem">{{ isEmpty(item.motorVehicle.plateClass) }}</div>
              <div class="natureItem">{{ isEmpty(item.motorVehicle.plateColor) }}</div>
              <div class="natureItem">{{ isEmpty(item.motorVehicle.plateNo) }}</div>
              <div class="natureItem">{{ isEmpty(item.motorVehicle.vehicleColor) }}</div>
              <!-- <div class="natureItem">{{ isEmpty(item.motorVehicle.behavior) }}</div>
              <div class="natureItem">{{ isEmpty(item.motorVehicle.vehicleFrontItem) }}</div>
              <div class="natureItem">{{ isEmpty(item.motorVehicle.vehicleRearItem) }}</div>
              <div class="natureItem">{{ isEmpty(item.motorVehicle.filmColor) }}</div>-->
              <div class="natureItem operation">
                <div
                  class="addNature searchBtn"
                  v-show="getshowPersonCase"
                  @click="ajaxAddCase(item.caseUuid,true)"
                >
                  <img
            ...
            <!-- <div class="contrast" v-show="compareList.length == 0">比对中<span>{{ value }}</span></div> -->
          </div>
        </div>
        ...
        <div class="caseListItem caseListItemdel">
          <div class="caseListItemImg cursor">
            <img
              ...
              :src="dataCaseInfo.caseImgUrl"
              alt="暂无图片"
              v-show="dataCaseInfo.caseImgUrl"
            >
            <img
              style="width: 2.6rem;height: 1.7rem"
          ...
          <div class="caseListItemTxt">
            <div class="caseListItemTxtTitle">
              <span class="leftWrods">案件名称:</span>
              <span
                class="caseNavBar"
                :title="dataCaseInfo.caseName"
                style="width: 1.3rem;"
              >{{ dataCaseInfo.caseName }}</span>
            </div>
      ...
      <el-dialog width="30%" title="串并案保存" :visible.sync="innerVisible" append-to-body>
        <div class="totalbox">
          <div style="display: flex; color:#99c9fa">
          ...
          <div
            style="display: flex;width: 100%;height: 60%;margin-top: 0.2rem; color:#99c9fa"
            v-if="innerVisible"
          >
            <div style="width: 0.6rem; color:#99c9fa">案件描述:</div>
            <textarea
              style="width: calc(100% - 1rem);height:1.2rem;border: 0.01rem solid #E8F0F4; margin-left:0.2rem; color:#99c9fa"
              v-model="description"
              placeholder="请输入案件描述"
            ></textarea>
          </div>
...
export default {
  ...
  methods: {
    ...
    getCompareList() {
      let data = {
        ...
        targetId: this.carNatureObj.vehicleUuid,
  ...
  computed: {
    ...mapGetters(["getshowPersonCase"]),
    dialogVisible: {
      get: function() {
        return this.$store.state.carNatureDialog;
      },
      set: function(newVal) {
        this.$store.state.carNatureDialog = newVal;
      }
    }
  },
  watch: {
    ...
    caseUuid: function(newValue, oldValue) {
      return newValue;
    }
  },
  mounted() {
    console.log(this.caseUuid);
  }
};
</script>
