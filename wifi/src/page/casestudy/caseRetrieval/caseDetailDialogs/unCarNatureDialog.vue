 ...
 * @LastEditTime: 2020-06-30 11:20:15
 * @LastEditors: chenxf
 * @Description:
 * @FilePath: \vacp-wfes\src\page\casestudy\caseRetrieval\caseDetailDialogs\unCarNatureDialog.vue
 * @
 -->
<template>
      ...
      <div class="dialogContainer">
        ...
        <div class="dialogDetail">
          <div class="detailItemLeft">
            <div class="natureName">骑车人属性</div>
            <!-- <div class="natureName naturePicture">线索截图</div> -->
            <div class="natureName">案件名称</div>
            <div class="natureName">案发时间</div>
            <div class="natureName">目标年龄段</div>
            <div class="natureName">目标包类型</div>
            <div class="natureName">目标包颜色</div>
            <div class="natureName">目标车颜色</div>
            <div class="natureName">目标是否携物</div>
            <div class="natureName">目标下衣类型</div>
            <div class="natureName">目标下衣颜色</div>
            <div class="natureName">目标驾驶人数</div>
            <div class="natureName">目标眼部特征</div>
            <div class="natureName">目标发型</div>
            <div class="natureName">目标嘴部特征</div>
            <div class="natureName">目标民族</div>
            <div class="natureName">目标围脖</div>
            <div class="natureName">目标性别</div>
            <div class="natureName">目标体态</div>
            <div class="natureName">目标是否打伞</div>
            <div class="natureName">目标上衣类型</div>
            <div class="natureName">目标上衣颜色</div>
            <div class="natureName">目标上衣纹理</div>
            <div class="natureName">目标朝向</div>
          ...
          <div class="detailItemRight">
            <div class="ownNature">
              ...
              <!-- <div class="natureItem naturePicture cursor" @click="lookCase(dataCaseInfo.caseUuid)">
                <img style="width: 100%;height: 100%" :src="unCarNatureObj.traceImgUrl" alt="暂无图片" v-show="unCarNatureObj.traceImgUrl">
                <img style="width: 100%;height: 100%" src="@/assets/error_none.png" alt="暂无图片" v-show="!unCarNatureObj.traceImgUrl">
              </div> -->
              <div class="natureItem">
                <span :title="dataCaseInfo.caseName">{{ dataCaseInfo.caseName }}</span>
              </div>
              <div class="natureItem">{{ dataCaseInfo.timeLowLimit }}</div>
              <div class="natureItem">{{ isEmpty(unCarNatureObj.ageType) }}</div>
              <div class="natureItem">{{ isEmpty(unCarNatureObj.bagStyle) }}</div>
              <div class="natureItem">{{ isEmpty(unCarNatureObj.bagColor) }}</div>
              <!-- <div class="natureItem">{{ isEmpty(unCarNatureObj.plateColor) }}</div> -->
              <div class="natureItem">{{ isEmpty(unCarNatureObj.vehicleColor) }}</div>

              <div class="natureItem">{{ isEmpty(unCarNatureObj.carryingType) }}</div>
              <div class="natureItem">{{ isEmpty(unCarNatureObj.trousersStyle) }}</div>
              <div class="natureItem">{{ isEmpty(unCarNatureObj.trousersColor) }}</div>
              <div class="natureItem">{{ isEmpty(unCarNatureObj.driverCount,true) }}</div>
              <div class="natureItem">{{ isEmpty(unCarNatureObj.eyePart) }}</div>
              <div class="natureItem">{{ isEmpty(unCarNatureObj.hairStyle) }}</div>
              <div class="natureItem">{{ isEmpty(unCarNatureObj.mouthPart) }}</div>
              <div class="natureItem">{{ isEmpty(unCarNatureObj.ethicCode) }}</div>
              <div class="natureItem">{{ isEmpty(unCarNatureObj.scarf) }}</div>
              <div class="natureItem">{{ isEmpty(unCarNatureObj.genderCode) }}</div>
              <div class="natureItem">{{ isEmpty(unCarNatureObj.bodyType) }}</div>
              <div class="natureItem">{{ isEmpty(unCarNatureObj.umbrellaFlag) }}</div>
              <div class="natureItem">{{ isEmpty(unCarNatureObj.coatStyle) }}</div>
              <div class="natureItem">{{ isEmpty(unCarNatureObj.coatColor) }}</div>
              <div class="natureItem">{{ isEmpty(unCarNatureObj.coatTexture) }}</div>
              <div class="natureItem">{{ isEmpty(unCarNatureObj.viewType) }}</div>
            ...
            <div class="othersNature" v-for="(item, index) in compareList"  :key="index">
              ...
              <!-- <div class="natureItem naturePicture cursor" @click="lookCase(item.caseUuid)">
                <img style="width: 100%;height: 100%" :src="item.bicycle.traceImgUrl" alt="暂无图片" v-show="item.bicycle.traceImgUrl">
                <img style="width: 100%;height: 100%" src="@/assets/error_none.png" alt="暂无图片" v-show="!item.bicycle.traceImgUrl">
              </div> -->
              <div class="natureItem">
                <span :title="item.caseName">{{ item.caseName }}</span>
              </div>
              <div class="natureItem">{{ item.timeLowLimit }}</div>
              <div class="natureItem">{{ isEmpty(item.bicycle.ageType) }}</div>
              <div class="natureItem">{{ isEmpty(item.bicycle.bagStyle) }}</div>
              <div class="natureItem">{{ isEmpty(item.bicycle.bagColor) }}</div>
              <!-- <div class="natureItem">{{ isEmpty(item.bicycle.plateColor) }}</div> -->
              <div class="natureItem">{{ isEmpty(item.bicycle.vehicleColor) }}</div>

              <div class="natureItem">{{ isEmpty(item.bicycle.carryingType) }}</div>
              <div class="natureItem">{{ isEmpty(item.bicycle.trousersStyle) }}</div>
              <div class="natureItem">{{ isEmpty(item.bicycle.trousersColor) }}</div>
              <div class="natureItem">{{ isEmpty(item.bicycle.driverCount,true) }}</div>
              <div class="natureItem">{{ isEmpty(item.bicycle.eyePart) }}</div>
              <div class="natureItem">{{ isEmpty(item.bicycle.hairStyle) }}</div>
              <div class="natureItem">{{ isEmpty(item.bicycle.mouthPart) }}</div>
              <div class="natureItem">{{ isEmpty(item.bicycle.ethicCode) }}</div>
              <div class="natureItem">{{ isEmpty(item.bicycle.scarf) }}</div>
              <div class="natureItem">{{ isEmpty(item.bicycle.genderCode) }}</div>
              <div class="natureItem">{{ isEmpty(item.bicycle.bodyType) }}</div>
              <div class="natureItem">{{ isEmpty(item.bicycle.umbrellaFlag) }}</div>
              <div class="natureItem">{{ isEmpty(item.bicycle.coatStyle) }}</div>
              <div class="natureItem">{{ isEmpty(item.bicycle.coatColor) }}</div>
              <div class="natureItem">{{ isEmpty(item.bicycle.coatTexture) }}</div>
              <div class="natureItem">{{ isEmpty(item.bicycle.viewType) }}</div>
              <div class="natureItem operation">
                <div class="addNature searchBtn"  v-show="getshowPersonCase" @click="ajaxAddCase(item.caseUuid,true)"><img style="margin-top: 0.02rem;margin-right: 0.05rem" src="../../../../assets/case/caseRetrievalSearch-add-icon.png" alt="">添加</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="caseListTitle">案件列表</div>
          <div class="caseListItem caseListItemdel">
            ...
            <div class="caseListItemTxt">
              <div class="caseListItemTxtTitle"><span class="leftWrods">案件名称:</span><span class="caseNavBar" :title="dataCaseInfo.caseName" style="width: 1.3rem;">{{ dataCaseInfo.caseName }}</span></div>
        ...
        <span slot="footer" class="dialog-footer">
          ...
          <el-button class="passBtn" v-show="getshowPersonCase" @click="checkcaseList()">保存串并</el-button>
      </span>
          <el-dialog
            width="30%"
            title="串并案保存"
  ...
  export default {
    ...
    methods: {
     ...
     getCompareList() {
          ...
          targetId:this.unCarNatureObj.bicycleUuid,
          pageNum:this.currentPage,//当前页
          pageSize:this.pageSize,//当前页结果数量
        };
        data = this.$qs.stringify(data);
        this.$http.post('pca/target/comparison/bicycleAttr',data)
        .then(res => {
    ...
    computed: {
      ...
      dialogVisible: {
        get: function () {
          return this.$store.state.unCarNatureDialog
        },
        set: function (newVal) {
          this.$store.state.unCarNatureDialog = newVal
        }
      }
    },
    watch: {
