    ...
    >
      <div class="videostrus" v-if="dialogVisible">
        ...
        <div class="strusbox" v-show="restltTotal > 0">
          <ul>
            ...
            >
              <p class="tagconvert bigimg" title="查看大图" @click.stop="lookBigImg(item, isoptfor)"></p>
              <p class="tagconvert transmitPic" title="转标注" @click.stop="confimTag(item, isoptfor)"></p>
              <p class="search searchPic" title="快捷以图搜图" @click.stop="searchimg(item,2)"></p>
              <p class="play" title="播放" @click="playvideo(item, index)"></p>
              <!-- <el-image class="img" :src="item.traceImgUrl"></el-image> -->
              <img
                :id="'sunimg'+index"
                class="imgbox"
              ...
              <!-- <img
                ...
                alt
              >-->
              <!-- <el-checkbox v-show="searchDownImg == 2"  @change="ClickCheckd(index)"  v-model="Checkd[index]"></el-checkbox> -->
              <!-- <i
                v-if="searchDownImg == 2"
                :class="Checkd.slice((currentPage-1)*pageSize,currentPage*pageSize)[index].state ? 'ONimg' : 'OFFimg'"
              ></i>-->
              <i v-if="searchDownImg == 2" :class="strusList[index].state ? 'ONimg' : 'OFFimg'"></i>
            </li>
          </ul>
        </div>
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button
          ...
          @click="batchDownLoad"
        >批量下载</el-button>
        <el-button type="primary" class="cancleBtn" @click="dialogVisible = false;clearAllinfo()">取 消</el-button>
        <el-button
          v-no-more-click
      ...
      <div class="taskrelation" v-if="invesdialogVisible">
        ...
        <div class="lookleft">
          ...
          <div class="loollcenter">
            <div class="treecenter">
              <el-radio-group :disabled="istwoinves == 2" v-model="opttype">
                <el-checkbox
                ...
                >全选</el-checkbox>
                <!-- <el-radio v-for="(item,index) in opttypes" @change="getptinfolist($event,index)" :label="item.value" :key="index">
                {{item.label}}-->
                <!-- <el-radio-group v-if="checkpoint == index" class="tworadio" :disabled="istwoinves == 2" v-model="deviceId"> -->
                <!-- <el-radio  :label="item.deviceUuid" :key="indexs" @change="getptlist($event,indexs)"> -->
                <!-- {{item.cameraName}} -->
                <el-checkbox-group
              ...
              <!-- <el-radio-group :disabled="istwoinves == 2" v-model="deviceId">
                  <el-radio v-for="(item,index) in ptlist" :label="item.deviceUuid" :key="index" @change="getptlist($event,index)">
                    {{item.cameraName}}
                    <el-checkbox-group v-if="checkpt == index" :disabled="istwoinves == 2" @change="getallVideo" v-model="checkvideo">
        ...
        <div class="lookright">
          ...
          <div class="imgcanvas" id="imgcanvas">
            <el-button class="searchBtn" :disabled="istwoinves == 2" @click.stop="clearCanvas">清除绘制</el-button>
            <img id="lookimg" :src="lableimg" alt>
            <canvas id="onecanvas" @click="getcoordnate" @dblclick="dbgetcoor"></canvas>
            <canvas id="twocanvas" v-show="showCanvas" @click="getcoordnate" style="z-index:1"></canvas>
            <!--  画箭头的canvas -->
          </div>

          <div class="labelinfo" v-show="showVideoStruct == 'true'">
            <p class="toptitle">
              <span>|</span>&nbsp;属性过滤
            </p>
            ...
            <div class="showinfo" v-show="isoptfor == 1">
              ...
              <p>
                ...
                <el-select
                  :disabled="issex"
                  @visible-change="getpropers(3,16)"
                  v-model="sex"
                  placeholder="请选择性别"
                >
                  <el-option label value></el-option>
              ...
              <!-- <p>
                  <span>民族：<i></i></span>
                  <el-select :disabled="isethnic" @visible-change="getpropers(3,19)" v-model="ethnic" placeholder="请选择民族">
                    <el-option label='' value=""></el-option>
              ...
              <p>
                ...
                <el-select
                  ...
                  @visible-change="getpropers(3,21)"
                  v-model="hairStyle"
                  placeholder="请选择发型"
                >
                  <el-option label value></el-option>
            ...
            <div class="showinfo" v-show="isoptfor == 2">
              ...
              <p>
                ...
                <el-select :disabled="isfaceMask" v-model="faceMask" placeholder="请选择面部是否遮挡">
                  <el-option label value></el-option>
                  <!-- <el-option v-for="(item,index) in propertyList" :label="item.valueString" :value="item.valueString" :key="index"></el-option> -->
              ...
              <p>
                ...
                <el-select
                  :disabled="isbodyRed"
                  @visible-change="getpropers(1,68)"
                  v-model="bodyRed"
                  placeholder="请选择车身红绳"
                >
                  <el-option label value></el-option>
            ...
            <div class="showinfo" v-show="isoptfor == 3">
              ...
              <p>
                ...
                <el-select
                  :disabled="issex"
                  @visible-change="getpropers(2,16)"
                  v-model="sex"
                  placeholder="请选择骑车人性别"
                >
                  <el-option label value></el-option>
                  <el-option
                    v-for="(item,index) in propertyList"
              ...
              <!-- <p>
                  <span>骑车人民族：<i></i></span>
                  <el-select :disabled="isethnic" @visible-change="getpropers(2,19)" v-model="ethnic" placeholder="请选择骑车人民族">
                    <el-option label='' value=""></el-option>
                    <el-option v-for="(item,index) in propertyList" :label="item.valueString" :value="item.valueString" :key="index"></el-option>
                  </el-select>
              </p>-->
          ...
          <zkyStructure @zkytab="zkytab" ref="zkyStruc" v-show="zkyStructureSwitch == 'true'"></zkyStructure>
        </div>
      </div>
      <span slot="footer" class="dialog-footer">
        <el-button
...
import getzkyinfo from "../../../api/realvideo/getzkyinfo.js"; //引入zky js
  ...
  methods: {
    ...
    closeDia() {
      if (this.bigfullScreen) {
        this.$nextTick(() => {
          ...
          this.$refs.dialogStrus.$el.querySelector(".el-dialog").style.height =
            "8.35rem";
          this.$refs.dialogStrus.$el.querySelector(".el-dialog").style.width =
            window.nowWidth + "px";
    ...
    openindia() {
      ...
      if (this.glststrusInfo.attrFlag == undefined) {
        ...
        //   this.beginData = this.getEarlierDate();   //选择开始时间
        ...
        //1：行人，2：机动车，3：非机动车
        this.$nextTick(() => {
          // this.opttype = this.glststrusInfo.type
          // this.opttype = this.deviceInfo.type;
          this.opttype = JSON.parse(sessionStorage.getItem("deviceInfo")).type;
          // this.getpointinfo(this.deviceInfo.type);
          ...
          if (this.showVideoStruct == "true") {
            if (this.glststrusInfo.attrFlag == 1) {
              this.disperson = false; //当前启用行人模块
              this.discar = true; //当前禁用车模块
              this.disbic = true; //当前禁用骑车人模块
              this.coatLength = this.glststrusInfo.personGlst.coatLength; //上衣款式
              this.coatPattern = this.glststrusInfo.personGlst.coatPattern; //上衣纹理      //人骑车人共用
              this.coatColor = this.glststrusInfo.personGlst.coatColor; //上衣颜色        //人骑车人共用
              this.trousersStyle = this.glststrusInfo.personGlst.trousersStyle; //下衣类别
              this.trousersColor = this.glststrusInfo.personGlst.trousersColor; //下衣颜色
              this.shoesStyle = this.glststrusInfo.personGlst.shoesStyle; //鞋子款式
              this.shoesColor = this.glststrusInfo.personGlst.shoesColor; //鞋子颜色
              this.sex = this.glststrusInfo.personGlst.sex; //性别                  //人骑车人共用
              // this.ethnic = this.glststrusInfo.personGlst.ethnic;  //民族               //人骑车人共用
              this.headLandMark = this.glststrusInfo.personGlst.headLandMark; //头部标识物   //人骑车人共用
              this.accessoryItem = this.glststrusInfo.personGlst.accessoryItem; //附属物品
              this.hairStyle = this.glststrusInfo.personGlst.hairStyle; //发型
              this.age = this.glststrusInfo.personGlst.age; //年龄
              this.packageStyle = this.glststrusInfo.personGlst.packageStyle; //包款式
              this.$commons.isEmpty(this.glststrusInfo.personGlst.coatLength)
                ? (this.iscoatLength = false)
                : (this.iscoatLength = true);
              this.$commons.isEmpty(this.glststrusInfo.personGlst.coatPattern)
                ? (this.iscoatPattern = false)
                : (this.iscoatPattern = true);
              this.$commons.isEmpty(this.glststrusInfo.personGlst.coatColor)
                ? (this.iscoatColor = false)
                : (this.iscoatColor = true);
              this.$commons.isEmpty(this.glststrusInfo.personGlst.trousersStyle)
                ? (this.istrousersStyle = false)
                : (this.istrousersStyle = true);
              this.$commons.isEmpty(this.glststrusInfo.personGlst.trousersColor)
                ? (this.istrousersColor = false)
                : (this.istrousersColor = true);
              this.$commons.isEmpty(this.glststrusInfo.personGlst.shoesStyle)
                ? (this.isshoesStyle = false)
                : (this.isshoesStyle = true);
              this.$commons.isEmpty(this.glststrusInfo.personGlst.shoesColor)
                ? (this.isshoesColor = false)
                : (this.isshoesColor = true);
              this.$commons.isEmpty(this.glststrusInfo.personGlst.sex)
                ? (this.issex = false)
                : (this.issex = true);
              // this.$commons.isEmpty(this.glststrusInfo.personGlst.ethnic) ? this.isethnic = false : this.isethnic = true
              this.$commons.isEmpty(this.glststrusInfo.personGlst.headLandMark)
                ? (this.isheadLandMark = false)
                : (this.isheadLandMark = true);
              this.$commons.isEmpty(this.glststrusInfo.personGlst.accessoryItem)
                ? (this.isaccessoryItem = false)
                : (this.isaccessoryItem = true);
              this.$commons.isEmpty(this.glststrusInfo.personGlst.hairStyl)
                ? (this.ishairStyle = false)
                : (this.ishairStyle = true);
              this.$commons.isEmpty(this.glststrusInfo.personGlst.age)
                ? (this.isage = false)
                : (this.isage = true);
              this.$commons.isEmpty(this.glststrusInfo.personGlst.packageStyle)
                ? (this.ispackageStyle = false)
                : (this.ispackageStyle = true);
            } else if (this.glststrusInfo.attrFlag == 2) {
              this.disperson = true; //当前禁用行人模块
              this.discar = false; //当前启用车模块
              this.disbic = true; //当前禁用骑车人模块
              this.vehicleAngle = this.glststrusInfo.motorVehicleGlst.vehicleAngle; //车辆角度     //车和骑车人共用
              this.vehicleClass = this.glststrusInfo.motorVehicleGlst.vehicleClass; //车辆型号     //车和骑车人共用
              this.vehicleColor = this.glststrusInfo.motorVehicleGlst.vehicleColor; //车辆颜色     //车和骑车人共用
              this.landMark = this.glststrusInfo.motorVehicleGlst.landMark; //车辆标识
              this.faceMask =
                this.glststrusInfo.motorVehicleGlst.faceMask == null
                  ? ""
                  : this.glststrusInfo.motorVehicleGlst.faceMask + ""; //面部遮挡        0：遮挡，1：为遮挡
              this.plateColor = this.glststrusInfo.motorVehicleGlst.plateColor; //车牌颜色
              this.plateClass = this.glststrusInfo.motorVehicleGlst.plateClass; //车牌类型
              this.specialVehicles = this.glststrusInfo.motorVehicleGlst.specialVehicles; // 特殊车辆
              this.vehicleBack = this.glststrusInfo.motorVehicleGlst.vehicleBack; // 车后盖
              this.bodyBreakage = this.glststrusInfo.motorVehicleGlst.bodyBreakage; // 车身破损
              this.roofGoods = this.glststrusInfo.motorVehicleGlst.roofGoods; // 车顶物件
              this.plateOcclusion = this.glststrusInfo.motorVehicleGlst.plateOcclusion; // 车牌遮挡
              this.bodyRed = this.glststrusInfo.motorVehicleGlst.bodyRed; // 车身红绳
              this.$commons.isEmpty(
                this.glststrusInfo.motorVehicleGlst.vehicleAngle
              )
                ? (this.isvehicleAngle = false)
                : (this.isvehicleAngle = true);
              this.$commons.isEmpty(
                this.glststrusInfo.motorVehicleGlst.vehicleClass
              )
                ? (this.isvehicleClass = false)
                : (this.isvehicleClass = true);
              this.$commons.isEmpty(
                this.glststrusInfo.motorVehicleGlst.vehicleColor
              )
                ? (this.isvehicleColor = false)
                : (this.isvehicleColor = true);
              this.$commons.isEmpty(
                this.glststrusInfo.motorVehicleGlst.landMark
              )
                ? (this.islandMark = false)
                : (this.islandMark = true);
              this.$commons.isEmpty(
                this.glststrusInfo.motorVehicleGlst.faceMask
              )
                ? (this.isfaceMask = false)
                : (this.isfaceMask = true);
              this.$commons.isEmpty(
                this.glststrusInfo.motorVehicleGlst.plateColor
              )
                ? (this.isplateColor = false)
                : (this.isplateColor = true);
              this.$commons.isEmpty(
                this.glststrusInfo.motorVehicleGlst.plateClass
              )
                ? (this.isplateClass = false)
                : (this.isplateClass = true);
              this.$commons.isEmpty(
                this.glststrusInfo.motorVehicleGlst.specialVehicles
              )
                ? (this.isspecialVehicles = false)
                : (this.isspecialVehicles = true);
              this.$commons.isEmpty(
                this.glststrusInfo.motorVehicleGlst.vehicleBack
              )
                ? (this.isvehicleBack = false)
                : (this.isvehicleBack = true);
              this.$commons.isEmpty(
                this.glststrusInfo.motorVehicleGlst.bodyBreakage
              )
                ? (this.isbodyBreakage = false)
                : (this.isbodyBreakage = true);
              this.$commons.isEmpty(
                this.glststrusInfo.motorVehicleGlst.roofGoods
              )
                ? (this.isroofGoods = false)
                : (this.isroofGoods = true);
              this.$commons.isEmpty(
                this.glststrusInfo.motorVehicleGlst.plateOcclusion
              )
                ? (this.isplateOcclusion = false)
                : (this.isplateOcclusion = true);
              this.$commons.isEmpty(this.glststrusInfo.motorVehicleGlst.bodyRed)
                ? (this.isbodyRed = false)
                : (this.isbodyRed = true);
            } else if (this.glststrusInfo.attrFlag == 3) {
              this.disperson = true; //当前禁用行人模块
              this.discar = true; //当前禁用车模块
              this.disbic = false; //当前启用骑车人模块
              this.backpackClass = this.glststrusInfo.bicycleGlst.backpackClass; //骑车人包类型\
              this.manned = this.glststrusInfo.bicycleGlst.manned; //违法载人
              this.coatPattern = this.glststrusInfo.bicycleGlst.coatPattern; //上衣纹理
              this.vehicleClass = this.glststrusInfo.bicycleGlst.vehicleClass; //车型
              this.headLandMark = this.glststrusInfo.bicycleGlst.headLandMark; //头部标识物
              this.vehicleAngle = this.glststrusInfo.bicycleGlst.vehicleAngle; //车辆角度
              this.sex = this.glststrusInfo.bicycleGlst.sex; //骑车人性别
              // this.ethnic = this.glststrusInfo.bicycleGlst.ethnic;  //骑车人民族
              this.coatColor = this.glststrusInfo.bicycleGlst.coatColor; //上衣颜色
              this.vehicleColor = this.glststrusInfo.bicycleGlst.vehicleColor; //车身颜色
              this.$commons.isEmpty(
                this.glststrusInfo.bicycleGlst.backpackClass
              )
                ? (this.isbackpackClass = false)
                : (this.isbackpackClass = true);
              this.$commons.isEmpty(this.glststrusInfo.bicycleGlst.manned)
                ? (this.ismanned = false)
                : (this.ismanned = true);
              this.$commons.isEmpty(this.glststrusInfo.bicycleGlst.coatPattern)
                ? (this.iscoatPattern = false)
                : (this.iscoatPattern = true);
              this.$commons.isEmpty(this.glststrusInfo.bicycleGlst.vehicleClass)
                ? (this.isvehicleClass = false)
                : (this.isvehicleClass = true);
              this.$commons.isEmpty(this.glststrusInfo.bicycleGlst.headLandMark)
                ? (this.isheadLandMark = false)
                : (this.isheadLandMark = true);
              this.$commons.isEmpty(this.glststrusInfo.bicycleGlst.vehicleAngle)
                ? (this.isvehicleAngle = false)
                : (this.isvehicleAngle = true);
              this.$commons.isEmpty(this.glststrusInfo.bicycleGlst.sex)
                ? (this.issex = false)
                : (this.issex = true);
              // this.$commons.isEmpty(this.glststrusInfo.bicycleGlst.ethnic) ? this.isethnic = false : this.isethnic = true
              this.$commons.isEmpty(this.glststrusInfo.bicycleGlst.coatColor)
                ? (this.iscoatColor = false)
                : (this.iscoatColor = true);
              this.$commons.isEmpty(this.glststrusInfo.bicycleGlst.vehicleColor)
                ? (this.isvehicleColor = false)
                : (this.isvehicleColor = true);
            }
          } else if (this.zkyStructureSwitch == "true") {
                ...
                for (let i = 0; i < this.strusList.length; i++) {
                  for (let k = 0; k < this.groundIds.length; k++) {
                    if (this.groundIds[k] == this.strusList[i].personUuid) {
                      ...
                      this.groundIds[k] == this.strusList[i].vehicleUuid
                    ) {
                      this.strusList[i].state = true;
                    } else if (
                      this.groundIds[k] == this.strusList[i].bicycleUuid
                    ) {
                      this.strusList[i].state = true;
                    }
                  }
    ...
    investigate(type) {
      // console.log(type, "kaishi");
      // if(type == 1){
      //   if(this.personNum == 0 && this.bicycleNum == 0 && this.vehicleNum == 0){
      //     this.$notify({
      //       type: 'warning',
      //       message: '当前没有结构化结果！',
      ...
      //   }
      // }
      //设置当前页面是从任务管理中点进来的还是从案件库中点来的参数
      //1表示是从任务管理点进来的，2是从案件库过来的
      setTimeout(() => {
        // this.handleCheckAll(true)
